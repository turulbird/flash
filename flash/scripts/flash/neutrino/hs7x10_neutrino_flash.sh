#!/bin/bash
# "-----------------------------------------------------------------------"
# "This script creates flashable images for Fortis HS7x10 receivers"
# "Author: Schischu/Audioniek"
# "Date: 07-06-2014"
#
# "Note: for HS7110  with loader 6.40, 6.46 or 6.47."
# "      &   HS7810A with loader 6.20, 6.26 or 6.27."
#
# "-----------------------------------------------------------------------"
...
OUTFILE="$BOXTYPE"_L6X0_"$IMAGE"_flash_R$RESELLERID.ird
...
#
#	 .name   = "Real_ROOT", //mtd5
#	 .size   = 0x01700000,	// 23M, 128k hole at 0x1AE0000 is used for force flash
...
#echo " dd if=./flash_tools/seedfile of=./tmp/DUMMY/dummy bs=1 skip=0 count=$FAKESIZE"
dd if=$TOOLSDIR/seedfile of=$TMPDUMDIR/dummy bs=1 skip=0 count=$FAKESIZE 2> /dev/null
#echo " MKSQUASHFS ./tmp/DUMMY ./mtd_fakeroot.bin -nopad -le"
...
#echo "MKSQUASHFS ./tmp/DUMMY ./tmp/mtd_fakedev.bin -nopad -le"
...
if [ "$IMAGE" == "kernel" ]; then
  ...
  #echo "SUMTOOL -p -l -e 0x20000 -i ./tmp/mtd_root.bin -o ./tmp/mtd_root.sum"
  ...
  #echo "PAD 0x1C00000 ./mtd_root.sum ./mtd_root.pad"
  $PAD 0x1C00000 $TMPDIR/mtd_root.sum $TMPDIR/mtd_root.pad
  echo " done."
  ...
  SIZED=`printf "%d" $SIZE`
  if [[ $SIZED > "29360128" ]]; then
    echo
    echo -e "\033[01;31m"
    echo "-- ERROR! -------------------------------------------------------------"
    echo
    echo " ROOT TOO BIG: 0x$SIZEH instead of 0x01C00000 bytes" > /dev/stderr
    echo
    echo " Exiting..."
    echo "-----------------------------------------------------------------------"
    echo -e "\033[00m"
  ...
  # Root part one size is 0x1700000, partition type 1 (Fake_APP, extending into Real_ROOT)
  #echo "dd if=./tmp/mtd_root.pad of=./tmp/mtd_root.bin bs=0x10000 skip=0x0000 count=0x0170"
  dd if=$TMPDIR/mtd_root.pad of=$TMPDIR/mtd_root.1.bin bs=65536 skip=0 count=368 2> /dev/null
  ...
  #echo "dd if=./tmp/mtd_root.pad of=./tmp/mtd_config.bin bs=0x10000 skip=0x0170 count=0x10"
  dd if=$TMPDIR/mtd_root.pad of=$TMPDIR/mtd_config.bin bs=65536 skip=368 count=16 2> /dev/null
  echo " done."

  echo -n "   + Part three: user partition..."
  # Root part three, max. size 0x1E00000, partition type 9 (User)
  #echo "dd if=./tmp/mtd_root.pad of=./tmp/mtd_user.bin bs=0x010000 skip=0x0180 count=0x040"
  dd if=$TMPDIR/mtd_root.pad of=$TMPDIR/mtd_user.bin bs=65536 skip=384 count=64 2> /dev/null
  echo " done."
fi
...
#  echo "  - HS7420 : 6.30, 6.36 or 6.37"
  ...
  echo " shows SCAN, or when you see activity on the USB stick."
