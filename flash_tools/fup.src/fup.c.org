...
//				printVerbose("File dummy.squash30.signed.padded deleted.\n");
	...
	{  // -rl(v): Change loader reseller ID
		...
		printverbose("IRD file contains a loader partition.\nLoader version is: V%X.%02X.%02X\n", (loaderSW >> 16) & 0xff, (loaderSW >> 8) & 0xff, loaderSW & 0xff);
		printverbose("Loader reseller ID is: %08X (%s)\n", loaderId, getModelName(loaderId));
		...
		||  (resellerId >>  8 != loaderId >>  8))
		{
			...
			||  (generation != 2 && generation != 3 && generation != 5) && (resellerId != loaderId))
			{
		...
		{
//				printverbose("Reseller ID to set: %08X\n", resellerIdL);
//				printverbose("Difference1: %08X\n", resellerIdL & 0xff00ff5f);
//				printverbose("Difference2: %08X\n", loaderId & 0xff00ff5f);
		...
		printVerbose("Changing loader reseller ID in IRD file %s\n", argv[2]);
		printVerbose("from %08X (%s)\n", loaderId, getModelName(loaderId));
		printVerbose("  to %08X (%s).\n", resellerIdL, getModelName(resellerIdL));
...
//		}
		...
		{
			pos = ftell(irdFile);
			len = readBlock(irdFile, argv[2], firstBlock, 1);
			firstBlock = 0;
			if (len < 0)
			{
				fclose(irdFile);
...
//			}
//		}
		printVerbose("Extracting IRD file %s succesfully completed.\n", argv[2]);
	}
