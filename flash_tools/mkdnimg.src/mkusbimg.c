...
#define BUF_SIZE    DATA_OFFSET  // must be same as data_offset???
...
struct _image_header
{
	...
	unsigned int data_offset;     /* offset of actual data */
...
//	fwrite(buf, 1, DATA_OFFSET, ofd);
	...
	for (a = 0; a < filesize;)
	{
		...
		if (readed == 0)
		{
			errprintf("File read error (input file %s).\n", infile);
			ret = -1;
			goto terminate;
		}
		...
		{
			errprintf("File write error (output file %s).\n", outfile);
			ret = -1;
			goto terminate;
		}
		a += readed;
	...
	/*
	 * move filepointer to beginning and write image header.
	 */
